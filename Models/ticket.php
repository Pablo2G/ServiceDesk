...
class Ticket
{
    ...
    //Lista todo los ticket del usuario que se le pase por parametro
    public static function listar($usuario)
    {
        $listaTickets = [];
        $db = Db::getConnect();
        $usuario;
        //Añadimos esta fila para reutilizar el codigo para admin ticket (todo)
        if ($usuario == "todo") {
    ...
    public static function crearTicket($usuario, $tipo, $descripcion)
    {
        ...
        //La fecha se pasa en formato Año-Mes-Día sino no la inserte el MySql
